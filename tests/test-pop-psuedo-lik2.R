...
for(i in 1:length(phi.seq)){
  phi <- phi.seq[i]
  V[i] <- myV_2(lam.tru = 1, phi.tru = .5, lam.free = 1, phi.free = phi)
}
plot(V~phi.seq)
...
for(i in 1:length(lam.seq)){
  lam <- lam.seq[i]
  V[i] <- myV_2(lam.tru = 1, phi.tru = .5, lam.free = lam, phi.free = .5)
}
plot(V~lam.seq)
...
V.tru <- myV_2(lam.tru = 1, phi.tru = .5, lam.free = 1, phi.free = .5)
M[1, ] <- c(1, .5, V.tru)
for(i in 1:length(lam.seq)){
  print(i)
  for(j in 1:length(phi.seq)){
    lam <- lam.seq[i]
    phi <- phi.seq[j]
    V <- myV_2(lam.tru = 1, phi.tru = .5, lam.free = lam, phi.free = phi)
