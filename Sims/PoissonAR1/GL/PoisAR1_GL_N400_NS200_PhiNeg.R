...
df9 = data.frame(matrix(ncol = 8, nrow = 200))

#Create columns lam.est, phi.est, estim.method, n, phi, phi.se, lam, lam.se
names(df9) = c('lam.est', 'phi.est', 'estim.method', 'n', 'phi.true', 'phi.se', 'lam.true', 'lam.se')
...
# save results
save(df9, file = sprintf("C:/Users/Stef/Desktop/countsFun/Sims/PoissonAR1/GL/PoisAR%s_GL_N%s_NS%s_Phi%s.RData", ARorder, n, nsim, PhiSign))
rm(list = ls())
